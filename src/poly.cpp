...
#include <algorithm>
    ...
    bool mycompgd(gd pr1, gd pr2)
    {
        ...
        {
            if (pr1.getd() > pr2.getd())
                return 1; /// removed >= Angers 08/04/2019
            else
                return 0;
        }
    }
    ...
    int compgd(const void *p1, const void *p2)
    {
        gd *pr1 = (gd *)p1;
        gd *pr2 = (gd *)p2;
        if (pr1[0].getg() < pr2[0].getg())
            return -1;
        if (pr1[0].getg() > pr2[0].getg())
            return 1;
        else
        {
            if (pr1[0].getd() > pr2[0].getd())
                return -1; /// removed >= Angers 08/04/2019
            else
                return 0;
        }
    ...
    { // can=0;
        ...
        if (can == 3)
            simple = 1; // déja simplifié on marque simple
    }
    ...
    void poly::onlysimpli() // simplification du polynome
    {
        ...
        if (data)
        {
            ...
            if (data[n - 1].getg() == infinit && data[n - 1].getd() != _infinit)
                popj(n - 1);

            // si le premier element vaut (x, -00) <=> +00,-00 on l'ote
            if (data[0].getd() == _infinit && data[0].getg() != infinit)
                popj(0);
...

    {
        ...
        gd r(m1);
        if (r.getg() == infinit && r.getd() == _infinit)
            return;
