...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
	...
	serie &serie::operator=(const poly &p1) // initialise avec un polynome cast polynome->serie
	{
		...
		q = p.getpol(p.getn() - 1);
		p.pop();
		if (q.getpol(q.getn() - 1).getd() != infinit)
			r.init(0, 0);
		else
			r.init(0, infinit);
		canonise = 1;
	...
	serie oplus(serie &s1, serie &s2)
	{
		...
		if (s1.q.getpol(0).getg() == infinit)
			return (s2);
		if (s2.q.getpol(0).getg() == infinit)
			return (s1);
	...
	****/
	...
	{ // p1^p2 + q1.r1* ^ p2
		...
		if (s1.r.getd() == infinit)
		{
			...
			if (result.p.getpol(result.p.getn() - 1).getd() == infinit)
				result.r.init(0, infinit); // p2 est �alement une trajectoire
			else
				result.r.init(0, 0);
			result.canonise = 1;
			return (result);
	...
	serie inf(serie &s1, serie &s2)
	{
		...
		if (s1.q.getpol(0).getg() == _infinit)
			return (s2);
		if (s2.q.getpol(0).getg() == _infinit)
			return (s1);
		...
		if (s1.r.getg() == 0 && s2.r.getg() == 0)
		{
			...
			result.q = result.p.getpol(result.p.getn() - 1);
			result.p.pop();
			if (s1.r.getd() == infinit && s2.r.getd() == infinit)
				result.r.init(0, infinit);
			else
				result.r.init(0, 0);
			result.canonise = 1;
	...
	struct str_expand
	{					// structure for expanding series
