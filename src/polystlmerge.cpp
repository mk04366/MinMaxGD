...
namespace mmgd
{
	...
	{ // can=0;
		if (n1 < 1)
			std::cout << " probleme poly.init";
		affecte(n1, vect, can);
		if (can == 0)
			simpli(); // on trie et on simplifie
		if (can == 2)
			onlysimpli(); // déjà trié on simplifie seulement
	}
	...
	void poly::onlysimpli() // simplification du polynome
	{
		...
		if (data)
		{
			...
			if (data[n - 1].getg() == infinity && data[n - 1].getd() != _infinity)
				popj(n - 1);

			// si le premier ��ent vaut (x, -00) <=> +00,-00 on l'ote
			if (data[0].getd() == _infinity && data[0].getg() != infinity)
				popj(0);
			simple = 1;
		}
	}
	...
	void poly::popj(unsigned int j)
	{
		...
		if (data != NULL)
		{
			if (j < (n - 1))
				for (i = j; i < (n - 1); i++)
					data[i] = data[i + 1]; // si on ne supprime pas le dernier �ement
			if (j < n)
				pop(); // indice existant
		}
	}
...

	{
		...
		else // pas besoin de r�llouer
		{
			...
			{ // on regarde si le 1er vaut epsilon pour l'oter
				if ((data[0].getg() == infinity) && (data[0].getd() == _infinity))
					data[0] = m1;
				else
				{
					data[n] = m1;
	...
	*/
	...
	// appel p=otimes(p1,p2); est possible car on a redefini l'operateur =
	{
		...
		*/
		...
		if (temp == NULL)
		{
			std::cout << "poly::otimes(const poly &,const poly &) - 2" << std::endl;
			mem_limite l(9);
			throw(l);
		}
		...
		if (tampon == NULL)
		{
			std::cout << "poly::otimes(const poly &,const poly &) - 3" << std::endl;
			mem_limite l(9);
			throw(l);
		}
		...
		if (tabtaille == NULL)
		{
			std::cout << "poly::otimes(const poly &,const poly &) - 4" << std::endl;
			mem_limite l(9);
			throw(l);
		}
		...
		for (j = 0; j < p1->getn(); j++)
		{
			...
			if (temp[j] == NULL)
			{
				std::cout << "poly::otimes(const poly &,const poly &) - 5" << std::endl;
				mem_limite l(9);
				throw(l);
			}
		...
		for (j = 0; j < taillediv2; j++)
		{
			...
			if (tampon[j] == NULL)
			{
				std::cout << "poly::otimes(const poly &,const poly &) - 6" << std::endl;
				mem_limite l(9);
				throw(l);
			}
		}
		...
		while (nbpoly > 1)
		{
			...
			for (j = 0, k = 0; j < taillediv2; j++) // on alloue de nouveaux tableaux pour les futurs fusionn�
			{
				...
				if (tampon[j] == NULL)
				{
					std::cout << "poly::otimes(const poly &,const poly &) - 7" << std::endl;
					mem_limite l(9);
					throw(l);
				}
