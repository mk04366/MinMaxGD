...
namespace mmgd
{
	...
	serie &serie::operator=(const poly &p1) // initialise avec un polynome cast polynome->serie
	{
		...
		q = p.getpol(p.getn() - 1);
		p.pop();
		if (q.getpol(q.getn() - 1).getd() != infinity)
			r.init(0, 0);
		else
			r.init(0, infinity);
		canonise = 1;
	...
	// mise sous forme canonique de la serie
	{
		...
		if (canonise == 1)
			return; // deja sous forme canonique
		...
		if ((nb_g >= r.getg()) || ((q.getpol(j).getd() - q.getpol(0).getd()) >= r.getd()))
		{
			...
			for (i = 0; i < j; i++) // pour tous les points du motif sauf le dernier
			{
				...
				for (n = 1; n <= nbcoups; n++)
					transitoire[k + n] = otimes(transitoire[k + n - 1], r); // ajout des nbcoups points
		...
		do
		{
			...
			j = 0;
			do
			{															  // on teste si le denier ��ent du transitoire est domin�par un ��ent du p�iodique
		...
		while (q.getpol(q.getn() - 1) == otimes(p.getpol(p.getn() - 1), r))
		{
			for (i = (q.getn() - 1); i > 0; i--)
				q.getpol(i) = q.getpol(i - 1);	  // on decale les element vers la queue
	...
	serie oplus(serie &s1, serie &s2)
	{
		...
		if (s1.q.getpol(0).getg() == infinity)
			return (s2);
		if (s2.q.getpol(0).getg() == infinity)
			return (s1);
		...
		*/
...

		{
			...
			{ // on commence par �endre le polyn�e
				...
				i--;
				if ((int)i < nj)
					nj--; // bug fixed june 2012 Rio de Janeiro nj will be used in the next we don't have to lose it
	...
	{ // p1^p2 + q1.r1* ^ p2
		...
		if (s1.r.getd() == infinity)
		{
			...
			if (result.p.getpol(result.p.getn() - 1).getd() == infinity)
				result.r.init(0, infinity); // p2 est �alement une trajectoire
			else
				result.r.init(0, 0);
			result.canonise = 1;
			return (result);
	...
	serie inf(serie &s1, serie &s2)
	{
		...
		if (s1.q.getpol(0).getg() == _infinity)
			return (s2);
		if (s2.q.getpol(0).getg() == _infinity)
			return (s1);
		...
		if (s1.r.getg() == 0 && s2.r.getg() == 0)
		{
			...
			result.q = result.p.getpol(result.p.getn() - 1);
			result.p.pop();
			if (s1.r.getd() == infinity && s2.r.getd() == infinity)
				result.r.init(0, infinity);
			else
				result.r.init(0, 0);
			result.canonise = 1;
	...
	*/
	...
	serie odot(serie &s1, serie &s2)
	{
		...
		{
			if (i == 0)
				result.p = p3.getpol(i);
			else
				result.p.add(p3.getpol(i));
			i++;
		}
	...
	{ // p1\odot p2 + q1.r1* \odot p2
		...
		if (s1.r.getd() == infinity)
		{
			...
			if (result.p.getpol(result.p.getn() - 1).getd() == infinity)
				result.r.init(0, infinity); // p2 est egalement une trajectoire
			else
				result.r.init(0, 0);
			result.canonise = 1;
			return (result);
	...
	serie fracodotsharp(serie &num, serie &den)
	{
		...
		if (num.r.getg() == 0 && den.r.getg() == 0)
		{
			...
			result.q = result.p.getpol(result.p.getn() - 1);
			result.p.pop();
			if (num.r.getd() == infinity && den.r.getd() == infinity)
				result.r.init(0, infinity);
			else
				result.r.init(0, 0);
			result.canonise = 1;
		...
		if (result.q.getn() > 1)
			result.q.pop(); // we remove the last point
		else
		{
			if (!(result.p == epsilon))
				result.q = result.p;
